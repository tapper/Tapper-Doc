...
C<Tapper> is a B<Modular test infrastructure, inclusive Operating
System and Virtualization support>. See
L<Tapper::Doc::FactSheet|Tapper::Doc::FactSheet> for a feature
overview.
...
=head3 B<TODO: PREPARE THESE PACKAGE FILES>
...
Create an Tapper config in C</etc/tapper.cfg>. Copy an example
config from C</tmp/tapper-example-configs/etc/tapper.cfg> and change
it to fit your needs.

=head2 Prepare central workdir

Create sub directories that appear in the Tapper config. You don't
need to understand them now. Just create the subdir hierarchy:
...
The manual L<prepare-netboot-image.pod|prepare-netboot-image.pod>
tells you how to configure such an image.
...
way to get the test results. Therefore, we recommend to install it
...
features, we still describe a setup of two test machines. Substitue
